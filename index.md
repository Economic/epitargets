...
It creates your target plus a companion `_date` target that records when
it ran:
...
adds an age-based cue so the target automatically re-runs after a time
...
gathers `_date` targets into a single tibble so you can see at a glance
